...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    if (!organizationId || !(await requireOrgPermissions(organizationId, req.user, res, ["billing:manage"]))) return;
...
router.post("/record-payment", async (req: Request, res: Response) => {
  try {
    ...
    if (!sub?.organization_id || !(await requireOrgPermissions(sub.organization_id, req.user, res, ["billing:manage"])))
      return;
...
router.post("/sync", async (req: Request, res: Response) => {
  try {
    ...
    if (!(await requireOrgPermissions(sub.organization_id, req.user, res, ["billing:read"]))) return;
