/**
 * Proxy route to serve stored files from S3 with auth and caching.
 * GET /api/files/:key — key is URL-decoded (e.g. logos/org-123.png, emails/emailId/attId-file.pdf).
...
router.get("/*", requireAuth, async (req: Request, res: Response) => {
  ...
  const orgId = await getOrganizationIdForKey(key, req.user!.id);
  if (!orgId) {
