...
export default function OrganizationSupportPage() {
  ...
  useEffect(() => {
    ...
    if (session && organizationSlug) {
      fetchOrganizationAndTickets()
    }
  }, [session, isPending, organizationSlug])

  const fetchOrganizationAndTickets = async () => {
    setLoading(true)
    try {
      ...
      console.error("Error fetching organization and tickets:", error)
  ...
  if (isPending) {
    return (
      <AppPageContainer fullWidth>
        <div className="mb-6">
          <Skeleton className="h-7 w-40 sm:w-48 mb-2" />
      ...
      <div className="mb-4 sm:mb-6">
        <div className="flex items-center gap-2.5 sm:gap-3 mb-2">
          ...
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-foreground truncate">Support</h1>
            <p className="text-muted-foreground text-sm">Tickets and help</p>
          </div>
        </div>
      </div>

      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4 sm:space-y-6">
        <TabsList className="grid w-full grid-cols-2 h-11 sm:h-12 rounded-xl p-1">
          <TabsTrigger value="tickets" className="rounded-lg text-sm sm:text-base app-touch min-h-0">Tickets</TabsTrigger>
          <TabsTrigger value="new" className="rounded-lg text-sm sm:text-base app-touch min-h-0">New ticket</TabsTrigger>
        </TabsList>
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                  <div className="flex-1">
                    <div className="relative">
                      ...
                      <Input
                        placeholder="Search organization tickets..."
                        value={searchQuery}
            ...
            {loading ? (
              ...
              <Card className="app-card">
                <CardContent className="p-8 sm:p-12 text-center">
                  ...
                  <h3 className="text-lg font-semibold mb-2">No organization tickets found</h3>
              ...
              <div className="space-y-3 sm:space-y-4">
                {filteredTickets.map((ticket) => (
                  <Card key={ticket.id} className="app-card hover:shadow-md active:bg-muted/30 transition-all cursor-pointer"
                        onClick={() => router.push(`/${organizationSlug}/support/${ticket.id}`)}>
          ...
          <TabsContent value="new" className="space-y-4 sm:space-y-6">
            <Card className="app-card">
              <CardHeader className="px-4 sm:px-6">
                <CardTitle className="flex items-center gap-2">
                  ...
                  Create Organization Support Ticket
                </CardTitle>
                <CardDescription>
                  Describe your organization's issue and we'll help you resolve it as quickly as possible.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleCreateTicket} className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <Input
                      placeholder="Brief description of your organization's issue"
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      placeholder="Please provide detailed information about your organization's issue. Include any error messages, steps to reproduce, and what your team was trying to do."
            ...
            {/* Organization-specific Help Section */}
            <Card className="app-card">
              <CardHeader className="px-4 sm:px-6">
                <CardTitle className="flex items-center gap-2">
                  <Users className="h-5 w-5" />
                  Organization Support Categories
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
