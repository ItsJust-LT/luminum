...
import { ArrowLeft, Plus, Trash2, Loader2, Save, FileDown } from "lucide-react";
...
export default function EditInvoicePage() {
  ...
  function addItem() { setItems((prev) => [...prev, { description: "", quantity: 1, unit_price: 0, tax_percent: 0 }]); }
  function removeItem(index: number) { if (items.length > 1) setItems((prev) => prev.filter((_, i) => i !== index)); }

  const buildPayload = useCallback(() => ({
    invoiceNumber, date, dueDate: dueDate || undefined, currency, language,
    companyName, companyEmail: companyEmail || undefined, companyPhone: companyPhone || undefined,
  ...
  }), [invoiceNumber, date, dueDate, currency, language, companyName, companyEmail, companyPhone, companyVat, clientName, clientEmail, clientPhone, clientAddressLine1, clientCity, clientCountry, items, discountAmount, shippingAmount, taxInclusive, notes, terms]);
  ...
  async function handleSaveAndGenerate() {
    ...
    try {
      ...
      toast.success("Invoice updated and PDF generated");
      router.push(`/${slug}/invoices/${invoiceId}`);
    } catch (err: any) {
      toast.error(err?.message || "Failed to update invoice");
    } finally {
      setSaving(false); setGenerating(false);
    }
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <Card className="sticky top-6">
            ...
            <CardContent className="flex flex-col gap-3">
              <div className="flex justify-between text-sm"><span className="text-muted-foreground">Subtotal</span><span className="font-medium">{formatMoney(subtotal)}</span></div>
              {totalTax > 0 && <div className="flex justify-between text-sm"><span className="text-muted-foreground">Tax{taxInclusive ? " (included)" : ""}</span><span className="font-medium">{formatMoney(totalTax)}</span></div>}
              {discountAmount > 0 && <div className="flex justify-between text-sm"><span className="text-muted-foreground">Discount</span><span className="font-medium text-red-600">-{formatMoney(discountAmount)}</span></div>}
              {shippingAmount > 0 && <div className="flex justify-between text-sm"><span className="text-muted-foreground">Shipping</span><span className="font-medium">{formatMoney(shippingAmount)}</span></div>}
              <Separator />
              <div className="flex justify-between"><span className="font-semibold">Grand Total</span><span className="text-xl font-bold">{formatMoney(Math.max(0, grandTotal))}</span></div>
            </CardContent>
          </Card>
        </div>
      </div>
