...
type PaystackTransactionFilters = { from?: string; to?: string; status?: string; currency?: string; perPage?: number }

function processRevenueResponse(res: { transactions?: any[] }, _currencyFilter?: string) {
  ...
  const topCustomers = Object.values(byCustomer).sort((a, b) => b.totalAmount - a.totalAmount).slice(0, 10)
  const recentTransactions = [...successful].sort((a, b) => new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime()).slice(0, 20)
  return {
    totalRevenue,
...
export function RevenueDashboard({ onRefresh }: RevenueDashboardProps) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Filter transaction data to analyze revenue</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(analytics.totalRevenue, filters.currency || 'ZAR')}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(analytics.averageTransactionValue, filters.currency || 'ZAR')}
            </div>
            <p className="text-xs text-muted-foreground">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  {Object.entries(analytics.revenueByCurrency).map(([currency, amount]) => (
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Revenue breakdown by transaction status</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  {Object.entries(analytics.revenueByStatus).map(([status, amount]) => (
                    <div key={status} className="flex justify-between items-center">
                      ...
                      <Badge variant={status === 'success' ? 'default' : 'secondary'}>
                        {formatCurrency(amount as number, filters.currency || 'ZAR')}
                      </Badge>
                    </div>
                  ))}
                </div>
                ...
                {analytics.topCustomers.map((customer: any, index: number) => (
                  <div key={customer.customer} className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <div className="text-right">
                      <p className="font-bold">{formatCurrency(customer.totalAmount, filters.currency || 'ZAR')}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Overall transaction statistics</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-green-600">{analytics.successfulTransactions}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Failed:</span>
                    <span className="font-medium text-red-600">{analytics.failedTransactions}</span>
                  </div>
                  <div className="flex justify-between">
