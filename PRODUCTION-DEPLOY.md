...
- **Pull unauthorized:** Regenerate a PAT with `read:packages` and update `PROD_SERVER_GHCR_TOKEN`. On the server, run `echo NEW_TOKEN | docker login ghcr.io -u YOUR_ORG --password-stdin` and re-run compose.
